...
from extras.scripts import Script, StringVar, IPAddressWithMaskVar, ObjectVar, ChoiceVar, IntegerVar, TextVar, BooleanVar,MultiObjectVar
...
class New(Script):
    class Meta:
        ...
        description = "Generate standard CPE router configuration and configure uplink switch and linkage"
        field_order = ['business_name', 'uplink_site', 'skip_zabbix', 'skip_uplink_port', 'confirmation_email', 'comments', 'rack_mount']
    ...
    ZAPI = ZabbixAPI(url='http://172.16.101.51/api_jsonrpc.php/', user='Admin', password='zabbix')
    SNMP_COMMUNITY = "got"
    ###################END###################
    ...
    rack_mount = ChoiceVar(choices=((True,"Rack Mount"), (False,"Wall Mount")))
    ...
    #Create the device
        ...
        if( not data["skip_zabbix"]):
            ...
            ## Post to Zabbix API to create host in mikrotik group and ICMP template
            try:
                hostid = self.ZAPI.host.create(
                    host=self.business_name,
                    interfaces=dict(
                        type=2,
                        main=1,
                        useip=1,
                ...
                self.log_info("failed to configure zabbix")
            
        if(not data["skip_uplink_port"]):
            try:
                agg_switches = Device.objects.filter(site=data["uplink_site"], device_role_id=self.AGG_ROLE_ID, status="active")
                selected_interface =""
                for agg_switch in agg_switches:
                    ...
                    if selected_interface != "":
                        ...
                        cable.save()
                        self.log_info("uplink switch chosen. Port {0} on {1}".format(selected_interface.name,agg_switch.name))
                        break
        ...
        self.log_success("Created {0} .        Run ansible-playbook -i netbox_inventory.yml test.yml -e 'device_name={0}' to generate config".format(device.name))
